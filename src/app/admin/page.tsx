...
const StatCard = ({ icon: Icon, label, value, sub, color }: { icon: React.ElementType; label: string; value: number; sub?: string; color: string }) => (
  <div className={`bg-dark-card rounded-2xl p-6 border border-white/5`}>
    <div className="flex items-center justify-between mb-4">
      ...
      <TrendingUp className="w-4 h-4 text-warm-gray/40" />
    </div>
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard icon={MessageSquare} label="Messages" value={s.unread_messages} sub="unread" color="bg-green-500/10 text-green-400" />
        </div>
      )}
