...
const StatCard = ({ icon: Icon, label, value, sub, color, href }: StatCardProps) => (
  ...
  >
    <div className="flex items-center justify-between mb-4">
      ...
      <TrendingUp className="w-4 h-4 text-[#C8B9A6]/40 transition-colors duration-200 group-hover:text-[#C8B9A6]/70" />
    </div>
    <p className="text-3xl font-bold text-[#F8F4E9] font-heading">{value}</p>
    <p className="text-[#C8B9A6] text-sm mt-1">{label}</p>
    {sub && <p className="text-xs text-[#D4AF37] mt-1">{sub}</p>}
  </Link>
);
...
export default function AdminDashboard() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-heading text-3xl font-bold text-[#F8F4E9]">Dashboard</h1>
        <p className="text-[#C8B9A6] mt-1">Welcome back to Prinsa Café Admin</p>
      </div>
